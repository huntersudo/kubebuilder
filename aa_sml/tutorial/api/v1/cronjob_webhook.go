...
// 我们用 kubebuilder 标记去生成 webhook 清单。 这个标记负责生成一个 mutating webhook 清单。
...
// ValidateCreate, ValidateUpdate 和 ValidateDelete 方法期望在创建、更新和删除时 分别验证其接收者。
// 我们将 ValidateCreate 从 ValidateUpdate 分离开来以允许某些行为，像 使某些字段不可变，以至于仅可以在创建的时候去设置它们。
//我们也将 ValidateDelete 从 ValidateUpdate 分离开来以允许在删除的时候的不同验证行为。
//然而，这里我们只对 ValidateCreate 和 ValidateUpdate 用相同的共享验证。
//在 ValidateDelete 不做任何事情，因为我们不需要再 删除的时候做任何验证。
...
func (r *CronJob) validateCronJob() error {
	...
	//apierrors.NewInvalid(qualifiedKind schema.GroupKind, name string, errs field.ErrorList) *StatusError
	return apierrors.NewInvalid(
		schema.GroupKind{Group: "batch.tutorial.kubebuilder.io", Kind: "CronJob"},
		r.Name, allErrs)
}

// OpenAPI schema 声明性地验证一些字段。 你可以在 API 中发现 kubebuilder 验证标记(前缀是 // +kubebuilder:validation)。
// 你可以通过运行 controller-gen crd -w 或者 这里 查找到 kubebuilder支持的用于声明验证的所有标记。
...
func (r *CronJob) validateCronJobName() *field.Error {
	if len(r.ObjectMeta.Name) > validationutils.DNS1035LabelMaxLength-11 {
		...
		// 报错的方式  Invalid(field *Path, value interface{}, detail string) *Error
